...
func fetchData(day int) ([]model.OHLCData, error) {
	url := fmt.Sprintf("https://api.coingecko.com/api/v3/coins/bitcoin/ohlc?vs_currency=usd&days=%d&precision=1", day)
	fmt.Println(url)
	...
	fmt.Println(string(body)) // Print response body for inspection
